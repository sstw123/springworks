...
import com.biz.memo.domain.CateVO;
...
public class MemoService {
	...
	public MemoDTO findBySeq(long m_seq) {
		return memoDao.findBySeq(m_seq);
	}
	...
	public List<CateVO> getCates() {
		...
		cates.add(CateVO.builder().cateName("오늘 할 일").cateValue("today").build());
		cates.add(CateVO.builder().cateName("약속").cateValue("promise").build());
		cates.add(CateVO.builder().cateName("중요메모").cateValue("tops").build());
