...
<article>
	<ul class="pagination">
		...
		<c:forEach begin="${pageDTO.startPageNo}" end="${pageDTO.endPageNo}" var="pageNo">
			<li class="page-item <c:if test="${pageNo == pageDTO.currentPageNo}">active</c:if>"><a class="page-link" href="${rootPath}/${url}?search=${search}&currentPageNo=${pageNo}">${pageNo}</a></li>
		</c:forEach>
