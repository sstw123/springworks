...
public class PropsRestController {
	/*
	 * properties는 프로젝트의 여러 클래스들이 공통적으로 사용하기 위해 특별히 저장해놓은 변수들
	 * properties로 등록한 프로젝트 변수를 프로젝트에서 참조하여 사용하는 방법
	 * 
	 * @Value("문자열")
	 * 접근제한자 String 변수명
	 * => 프로젝트에 등록된 properties를 참조하여 해당 문자열을 검색하고
	 * 문자열이 검색되면 문자열=값 형식으로 저장된 곳에서 값을 가져와서 변수명에 저장하여
	...
	public String enc() {
		...
		String ret = String.format("naver : %s\nmysql : %s", naver_username, mysql_username);
