	...
	<script>
		$(function() {
			$("#btn-search").click(function() {
				...
				// $("form").serialize()
				// form에 입력된 데이터를 (서버에 전송할 수 있도록) 쿼리형식으로 문자열화 하기
				var query = $("form").serialize();
				
				
				/*
				Async JavaScript And XML : AJAX
				비동기방식으로 서버에 request를 보낼 때 사용하는 js의 내장 프로토콜이다
				1. web에서 ajax에게 request를 보내면
				2. ajax는 즉시 ok를 보내서(=200코드) web이 다른 작업을 수행할 수 있도록 하고
				3. ajax는 매개변수로 설정된 항목대로 서버에 request를 보낸다
				...
				*/
				$.ajax({
					url : "${rootPath}/searchAPI",	// 서버로 request를 보낼 주소
					data : query,					// 서버로 보낼 데이터
					type : 'post',					// request할 method
					// dataType : 'json',				// response로 받을 데이터 타입
					success : function(result) {	// 서버로 보낸 요청이 성공했을 때
						//resFunc(result)
						$("#bok-list").html(result)
					},
					error : function(xhr, status, error) { // 서버에서 오류코드를 보낼 때
						alert(error)
					}
				})
