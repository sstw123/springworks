...
public class TourController {
	...
	public String total(@RequestParam(value="t_city", required=false, defaultValue="1") String t_city,
		...
		List<CityVO> sigunList = taService.getAreaData(t_city);
		model.addAttribute("SI", sigunList);
		
		List<AreaBaseDTO> baseList = taService.getAreaBaseListData(t_city, t_sigun);
		model.addAttribute("BASELIST", baseList);
		
		List<CityVO> cityList = null;
		try {
			cityList = taService.getAreaData();
	...
	public String city(Model model) {
		...
		/*
		try {
			cityString = taService.getDataString();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
