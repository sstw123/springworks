...
<body>
	<header onclick="location.href='${rootPath}/'" style="cursor:pointer">
		<h3>이미지 갤러리</h3>
		<p><a id="logout" href="${rootPath}/member/logout">${MEMBER.u_id}</a></p>
	</header>
	...
	<div id="login_modal">
		<c:if test="${MODAL == 'JOIN'}">
			<%@ include file = "/WEB-INF/views/body/member-join.jsp" %>
		</c:if>
		<c:if test="${MODAL == 'LOGIN'}">
			<%@ include file = "/WEB-INF/views/body/member-login.jsp" %>
		</c:if>
	</div>
</body>
</html>